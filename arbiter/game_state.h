...
// RUBRIC: Correct Enforcement of Slot Constraints (Solar=10 + Lunar=10 fills all 20).
constexpr int INVENTORY_SLOTS = 20;
// RUBRIC: Long-Term Storage Handling & Retrieval - 64 LT slots per player.
...
constexpr float STUN_DURATION = 3.0f; // RUBRIC: exact 3-second stun duration
constexpr float ULTIMATE_PAUSE = 10.0f; // RUBRIC: exact 10-second ultimate window
...
// ─────────────────────────────────────────────
// RUBRIC: Memory Management - primary inventory + long-term storage.
// RUBRIC: Contiguous Allocation Strategy (find_contiguous below).
// RUBRIC: Fragmentation Handling (allocator evicts to LT to make a contiguous run).
struct Inventory {
...
struct Entity {
    ...
    void init_player(int idx, int roll_no, int num_players) {
        ...
        // HP = (roll_no % 1000) + rand(100..1000)  [spec Section 10]
        max_hp = (roll_no % 1000) + 100 + rand() % 901;
        hp = max_hp;
...
// RUBRIC: Resource Locking Mechanism (table_mutex + per-entry locked flag).
...
// RUBRIC: Deadlock Detection (Circular Wait) - DFS over this wait-for graph.
struct WaitForGraph {
...
// RUBRIC: Correct Use of Shared Memory (No Pipes) - all IPC via this struct.
// RUBRIC: Shared Memory Consistency under load - global_mutex serialises access.
// RUBRIC: Mutex/Semaphore Implementation - PROCESS_SHARED mutex + cond var below.
// RUBRIC: Race Condition Prevention - every entity/state mutation holds global_mutex.
struct SharedState {
    // ── Synchronization ──────────────────────
