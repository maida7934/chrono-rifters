...
static void print_menu(SharedState* s, int pidx) {
    ...
    // Inventory — show slot-by-slot
    hip_print("INVENTORY [20 slots]:\n  ");
...
static WeaponID pick_inventory_weapon(SharedState* s, int pidx) {
    ...
    if (c < 0 || c >= WPN_COUNT || !me.inventory.has((WeaponID)c)) { hip_print("Not found.\n"); return WPN_NONE; }
    return (WeaponID)c;
}
static WeaponID pick_lt_weapon(SharedState* s, int pidx) {
    Entity& me = s->entities[pidx];
    if (me.inventory.lt_count == 0) { hip_print("LT storage empty.\n"); return WPN_NONE; }
    hip_print("LT: ");
    for (int i = 0; i < me.inventory.lt_count; ++i)
        hip_print("%d=%s ", me.inventory.lt_storage[i], WEAPON_TABLE[me.inventory.lt_storage[i]].name);
    hip_print("Choice: ");
    int c = hip_read_int(-1);
    if (c < 0 || c >= WPN_COUNT) return WPN_NONE;
    for (int i = 0; i < me.inventory.lt_count; ++i)
        if (me.inventory.lt_storage[i] == c) return (WeaponID)c;
    hip_print("Not in LT.\n"); return WPN_NONE;
}
